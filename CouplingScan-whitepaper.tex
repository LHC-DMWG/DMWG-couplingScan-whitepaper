...
$g_{q,\text{lim}} = g_{q,\text{th}} \sqrt{\sigma_\text{obs}/\sigma_\text{th}}$, where $g_{q,\text{th}}$ is the value of $g_q$ in calculating $\sigma_\text{th}$. The equivalent for a dilepton final state, where the cross section scales proportionally to $g_qg_l$ and $g_q$ is fixed, is $g_{l,\text{lim}} = g_{l,\text{th}} (\sigma_\text{obs}/\sigma_\text{th})$.
...
% Commenting out as this is all tackled in separate section now.
...
{\color{red}TODO add brief statement here on why we need a grid: both mDM and mMED because both affect MET distribution/analysis acceptance}
