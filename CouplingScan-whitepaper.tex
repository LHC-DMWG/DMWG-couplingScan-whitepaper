...
The \texttt{couplingscan} package released here \ldots [add links and description of installation, depending on whether we just host as is on DMWG page or do something else]

Note on LHAPDF and installation.

Once installed in your environment, the code can be loaded in any python file via:
\begin{minted}{python}
from couplingscan.scan import *
from couplingscan.rescaler import *
\end{minted}

The code is structured around two classes: Scans and Rescalers. [Discuss.]
Any warnings of DO NOT DO's that are not expressly forbidden in code?

Integration warnings may occur at some points in this calculation. For cases observed so far, the impact appears to be negligible. However, if the reported values from integrals with warnings begin to diverge from their neighbours or from expectation, it would merit further investigation and possibly the adoption of a new integration method (or a range of options) in the package.
\end{document}
