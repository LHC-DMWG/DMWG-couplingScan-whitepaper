...
The goal of this paper is to establish techniques for converting analysis limits computed in the context of one set of simplified model parameters into any other desired set of model parameters, with reasonable accuracy without resorting to generating events. This will allow 
...
% On-shell decay to quarks is possible everywhere, so we can use on-shell approximations everywhere. At tree level just the one diagram with two quark couplings.
